...
const IDCards: React.FC = () => {
  ...
  // Load applicants from localStorage or mock
  useEffect(() => {
    setLoading(true);
  ...
  // Only show approved applicants
  const approvedApplicants = filteredApplicants.filter(applicant => applicant.status === 'approved');
  ...
  const handlePrint = (applicant: any) => {
    ...
    // Generate HTML with phone number included
        ...
        <style>
          @media print {
            ...
            .card {
              ...
              color: white; padding: 16px; border-radius: 8px; margin-bottom: 20px;
              overflow: hidden; box-sizing: border-box;
            }
            .logo-container { text-align: center; margin-bottom: 10px; }
            .logo-image { max-height: 40px; max-width: 100px; }
            .photo-container {
              width: 80px; height: 100px; border: 2px solid white; overflow: hidden; margin: 5px auto;
            }
            .photo-image { width: 100%; height: 100%; object-fit: cover; }
            .color-band {
              position: absolute; bottom: 0; left: 0; right: 0; height: 16px; display: flex;
            }
          ...
          <div class="card">
            <div style="display:flex; height:100%;">
              <div style="width:33%; display:flex; flex-direction:column; align-items:center; justify-content:space-between;">
                ...
                ${photo ? `<img src="${photo}" class="photo-image" />` : ''}
                <div style="margin-top:5px; background:#fcd116; padding:3px 8px; border-radius:2px; font-weight:bold; font-size:10px;">${applicant.visaType.toUpperCase()}</div>
              </div>
              <div style="width:67%; padding-left:10px; font-size:10px;">
  ...
  const handleDeleteConfirm = () => {
    if (applicantToDelete) {
      const updatedApplicants = applicants.filter(a => a.id !== applicantToDelete.id);
  ...
  return (
    <div className="space-y-6">
            ...
            <DialogContent className="sm:max-w-lg">
              ...
              <BulkPrintSelector 
                ...
                onPrintSelection={(selected) => {
                  try {
                    localStorage.setItem('selectedApplicantsForPrint', JSON.stringify(selected));
                    navigate('/id-cards/print');
                    toast.success(`${selected.length} applicants selected for printing`);
                  } catch {
      ...
      <Card>
        ...
        <CardContent>
          {/* Search and format select */}
          <div className="mb-4 flex flex-col sm:flex-row gap-3">
            <div className="relative flex-grow">
              <Search className="absolute left-2.5 top-2.5 h-4 w-4 text-gray-500" />
              <Input
                placeholder="Search by name, nationality, area..."
                className="pl-8"
                value={searchTerm}
                    ...
                    <CardContent className="p-4">
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <p className="text-sm"><strong>Phone:</strong> {applicant.phoneNumber || 'N/A'}</p>
                        </div>
                      ...
                      <div className="flex flex-col sm:flex-row gap-2 mt-4">
                        ...
                        <Button variant="outline" size="sm" onClick={() => handlePrint(applicant)}>
                          <Printer className="h-4 w-4 mr-1" /> Print {printFormat}
                        </Button>
                        <Button variant="outline" size="sm" onClick={() => handleDeleteClick(applicant)}>
                          ...
                          <div className="flex gap-2">
                            ...
                            <Button variant="outline" size="sm" asChild>
                              <Link to={`/id-cards/${applicant.id}/print`}>
                                <Download className="h-4 w-4 mr-1" /> Print Page
                              </Link>
                            </Button>
                            <Button variant="outline" size="sm" onClick={() => handleDeleteClick(applicant)}>
                              <Trash2 className="h-4 w-4 mr-1 text-red-500" /> Delete
